...
function movieProcess(result){
    return new Promise(async (resolve,reject)=>{
        try {
            ...
            //packet[0]["children"][1]["children"].push(movie_obj);
            let movieNodeTag= await jsonToXMLParser.PrepareChildNode(movie_obj);
            writeStream.write(movieNodeTag);
            resolve(true);
        }
        catch (err) {
            reject(err.message);
